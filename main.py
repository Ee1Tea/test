...
def run():
        ...
        for i in range(len(ranked)):
            ...
            if ranked[i]['queueType'] == 'RANKED_FLEX_SR':
                ...
                embed.add_field(name="FLEX",
                                value=f"{rank} {lp} lp\nПобед: {wins}\tПоражений: {lose}\tВинрейт: {'{:.2f}'.format(wins / (wins + lose) * 100)}%\n")
        embed.add_field(
            name=f"Последний раз в сети: {time.strftime('%d %b %Y %H:%M:%S', time.localtime(player['revisionDate'] / 1000))}",
            value="")
    ...
    async def last_game(ctx, *kwargs):
        ...
        current_game_url = f"https://europe.api.riotgames.com/lol/match/v5/matches/{games_list[0]}" + '?api_key=' + settings.LOL_API_SECRET
        ...
        # Вывод результата
        back.save(settings.GENERATED_IMAGE_DIR + f"{player['name']}_{games_list[0]}" + ".png")
